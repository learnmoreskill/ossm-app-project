...
import kotlinx.android.synthetic.main.activity_student_detail_attendance.*
 ...
 */
class StudentDetailAttendance : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_student_detail_attendance)
        supportActionBar!!.setDisplayHomeAsUpEnabled(true)
        sid = intent.getStringExtra("sid")
        sname = intent.getStringExtra("sname")
        this.title=sname+"'s Attendance"
    ...
    fun setAttendanceList(){
        ...
        request.setOperation(Constants.FETCH_SD_ATTENDANCE_OPERATION)
        request.setStudent(student)
        mCompositeDisposable?.add(requestInterface.operation(request)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(this::handleResponse, this::handleError))
        sd_view_attendance_progress.visibility=View.VISIBLE
    }
    ...
    private fun handleResponse(resp: ServerResponse) {
        ...
        if (resp.getResult() == Constants.SUCCESS) {
            ...
            var attendance=resp.getAttendance()
            for(date in attendance){
                var map = HashMap<String,String>()
                map.put(KEY_DATE,"{cmd-calendar} "+utils.prettifyDate(date.getDate()))
            ...
            sd_view_attendance_progress.visibility=View.GONE
            attendance_list_view.visibility=View.GONE
            tv_sd_attendance_error_msg.text=resp.getMessage()
            sd_attendance_error_card.visibility=View.VISIBLE
    ...
    private fun handleError(error: Throwable) {
        ...
        sd_view_attendance_progress.visibility=View.GONE
        attendance_list_view.visibility=View.GONE
        tv_sd_attendance_error_msg.text=error.localizedMessage
        sd_attendance_error_card.visibility=View.VISIBLE
