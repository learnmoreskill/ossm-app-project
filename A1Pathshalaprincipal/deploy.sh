...
cp app/build/outputs/apk/release/WPprincipal-*.apk app/build/outputs/apk/release/WPprincipal-"$version_name"_"$sha_hash".apk
apk=app/build/outputs/apk/release/WPprincipal-"$version_name"_"$sha_hash".apk
name=WPprincipal-"$version_name"_"$sha_hash".apk
...
tg_message="[$sha_hash](https://github.com/ishubhamsingh/WPprincipal_client/commit/$sha_hash) passed"
  else
  
tg_message="[$sha_hash](https://github.com/ishubhamsingh/WPprincipal_client/commit/$sha_hash) failed"
  fi
...
function sendfile() {
  curl --upload-file $apk https://transfer.sh/$name
